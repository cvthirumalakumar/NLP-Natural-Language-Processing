...
if __name__ == '__main__':
    if len(sys.argv) == 4:
        ...
        language_mode.py <smoothing> <corpus_path> <k>
        eg. generator.py i corpus.txt 3''')
        exit()
    if sys.argv[1] == 'n':
        smoothing = None
    else:
    ...
    print("Top",sys.argv[3],"choices for next word")
    for word,prob in lm.generate(text, int(sys.argv[3])).items():
        print(word,":",prob)