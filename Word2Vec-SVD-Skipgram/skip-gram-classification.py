...
class News_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        sample = self.embeddings['target_embeddings'][self.processed_text[idx]]+self.embeddings['context_embeddings'][self.processed_text[idx]]
...
def train(model, model_name, n_epochs=25):
  ...
  model_path = "skipgram_classification/skipgram-classification-"+model_name+".pt"
...
if __name__ == '__main__':
    ...
    val_accuracies = []
    test_accuracies = []
    word2idx,_ =  torch.load('svd/svd-word-vectors_1.pt')
    for file in glob.glob("skipgram/*"):
        window_size = file.split("-")[-1].split(".")[0]
        print(f"Window size {window_size}")
        print("Loading word vectros")
        word_vectors_tensor_t =  torch.load(file, map_location='cpu')
